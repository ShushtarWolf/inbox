...
<template>
  <div class="venus-page-stack">
    ...
    <div class="grid gap-3 sm:grid-cols-2">
      <div class="canva-panel p-4 text-center">
    ...
    <div v-for="s in data?.sessions" :key="s.id" class="canva-list-card p-3">
    ...
    <p v-if="!data?.sessions?.length" class="canva-panel border-dashed p-4 text-sm text-brand-gray-600">{{ $t('coach.noSessionsToday') }}</p>
    ...
    <section class="space-y-2">
      ...
      <div v-for="s in data?.upcomingSessions" :key="`upcoming-${s.id}`" class="canva-list-card p-3">
        <p class="font-bold">{{ s.athlete.name }}</p>
