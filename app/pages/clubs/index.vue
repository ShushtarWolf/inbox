    ...
    <div v-if="showMap" class="ios-card space-y-3 p-4">
      ...
      <div class="grid gap-3 lg:grid-cols-2">
        <a
          v-for="club in clubs"
          :key="`${club.id}-map`"
          :href="club.distanceKm != null ? `https://www.openstreetmap.org/?mlat=${route.query.lat}&mlon=${route.query.lng}#map=12/${route.query.lat}/${route.query.lng}` : `https://www.openstreetmap.org/search?query=${encodeURIComponent(localizedField(club, 'nameFa', 'nameEn'))}`"
          target="_blank"
          rel="noreferrer"
