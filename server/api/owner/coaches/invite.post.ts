...
export default defineEventHandler(async (event) => {
  ...
  const body = await readBody<{ email?: string; role?: StaffRole; name?: string }>(event)
  ...
  const membership = await prisma.staffMembership.upsert({
    ...
    create: {
      ...
      role,
      coachId: coach?.id,
      permissionsJson: JSON.stringify(role === 'COACH' ? ['schedule'] : ['calendar', 'crm']),
      active: true,
    },
    update: { active: true, coachId: coach?.id },
  })
