...
`deploy.yml` prefers a green **CI** run for the same commit (skips duplicate smoke rebuild). If CI has not succeeded for that SHA, it runs the auth smoke gate, then deploys. Concurrent deploys wait in line (`cancel-in-progress: false`).
