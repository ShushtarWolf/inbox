...
| `KAVENEGAR_TEMPLATE` | Strongly recommended for OTP | Panel-approved Verify Lookup template (e.g. `inbox-verify`). Body must be `code: %token%` + `کد تایید اینباکس` + `@inboxs.ir #%token2%` for iOS/Android autofill. Without it, OTP uses free-text `sms/send`. |
...
| `KAVENEGAR_TEMPLATE` | e.g. `inbox-verify` | **Preferred for OTP** — panel-approved Verify Lookup template. Autofill body: `code: %token%` / `کد تایید اینباکس` / `@inboxs.ir #%token2%` |
...
Wait until status is **عملیاتی**, then set `KAVENEGAR_TEMPLATE` to that template name and restart/redeploy. Phones keep the old `inbox: 123456 کد تایید` wording until this panel step is done.
